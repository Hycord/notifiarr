...
import { useReloadConfig, useMainConfig, useUpdateMainConfig, useExportConfig, useImportConfig } from '@/hooks/use-config-queries';
...
export default function SettingsPage() {
  const reloadConfig = useReloadConfig({
    onSuccess: (data) => {
      toast.success(`Configuration reloaded: ${data.summary.clients} clients, ${data.summary.servers} servers, ${data.summary.events} events, ${data.summary.sinks} sinks`);
    },
    onError: (error) => {
  ...
  const [exportBase64, setExportBase64] = useState<string>('');
  const exportConfig = useExportConfig({
    onSuccess: async (blob) => {
      setExportBlob(blob);
      // Convert blob to base64 for copy option
      try {
  ...
  useEffect(() => {
    if (mainConfig) {
      if (typeof mainConfig === 'string') {
        setCodeContent(mainConfig);
      } else {
        setCodeContent(JSON.stringify(mainConfig, null, 2));
      }
    }
  }, [mainConfig]);
  ...
  const handleRootSaveCode = () => {
    try {
      ...
      updateMainConfig.mutate(parsed);
    } catch (e) {
    ...
    try {
      if (importTab === 'file') {
        ...
        if (!importBase64.trim()) {
          toast.error('Please paste base64 config text');
          return;
        }
        const b64 = importBase64.trim().replace(/\s+/g, '');
        if (!isValidBase64(b64)) {
          toast.error('Invalid base64 content');
          return;
        }
        // Decode base64 to gzip blob and wrap as File
      ...
      toast.error(`Failed to import configuration: ${error?.message || String(error)}`);
    }
  };
  ...
  function isValidBase64(value: string) {
    ...
    try {
      // atob will throw if not valid base64
      atob(s);
      return true;
    } catch {
      return false;
  ...
  // Decode base64 (.json.gz) and upload as replace
  const handleClearConfirm = async () => {
    try {
      const binaryString = atob(BLANK_CONFIG);
      ...
      const blob = new Blob([bytes], { type: 'application/gzip' });
      // Wrap as File so backend sees a proper .json.gz name
      const file = new File([blob], 'blank-config.json.gz', { type: 'application/gzip' });
      // Reuse import mutation for consistent feedback
      await importConfig.mutateAsync({ file, mode: 'replace' });
      // Ensure runtime picks up changes immediately
      try {
      ...
      <div className="space-y-6 max-w-4xl px-4 sm:px-0">
        ...
        <Tabs value={configTab} onValueChange={setConfigTab} className="space-y-6">
          ...
          <TabsContent value="form" className="space-y-6">
            <Card>
              ...
              <CardContent>
                {mainLoading && (
                  <p className="text-sm text-muted-foreground">Loading root configuration…</p>
                )}
                {!mainLoading && (
                  <RootConfigForm
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Reload, import, and export configurations
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-medium">Reload Configuration</h4>
                <p className="text-sm text-muted-foreground">
                  Force reload all configuration files from disk
                </p>
              </div>
              <Button
            ...
            <div className="flex items-center justify-between border-t pt-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Download all configurations as a JSON bundle
                </p>
              </div>
              <Button 
                variant="outline" 
            ...
            <div className="flex items-center justify-between border-t pt-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Upload and restore configuration bundle (.json or .json.gz)
                </p>
              </div>
              <Button 
                variant="outline" 
            ...
            <div className="flex items-center justify-between border-t pt-4">
              <div>
                <h4 className="font-medium">Clear Configuration</h4>
                <p className="text-sm text-muted-foreground">
                  Replace all current configuration with a blank config
                </p>
              </div>
              <Button 
                variant="destructive"
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Backend API settings and authentication
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <div className="space-y-4">
                ...
                {importTab === 'base64' && (
                  <div className="space-y-2">
                    <p className="text-sm text-muted-foreground">Paste base64 representing a .json.gz bundle</p>
                    <textarea
                      value={importBase64}
                    ...
                    <div className="space-y-2 border-t pt-3">
                      <p className="text-sm text-muted-foreground">Or fetch base64 from any URL</p>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              const endpoint = `/api/fetch-raw?url=${encodeURIComponent(u)}`;
                              const res = await fetch(endpoint);
                              if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);
                              const text = await res.text();
                              const cleaned = text.trim().replace(/\s+/g, '');
                              if (!isValidBase64(cleaned)) {
                                toast.error('Fetched content is not valid base64');
                                return;
                              }
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center space-x-2 cursor-pointer">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Replace all existing configurations (recommended)
                        </p>
                      </div>
                    </label>
                    <label className="flex items-center space-x-2 cursor-pointer">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Merge with existing configurations (may cause conflicts)
                        </p>
                      </div>
                    </label>
                  </div>
                ...
                {importMode === 'replace' && (
                  <p className="text-sm text-amber-600 dark:text-amber-500">
                    Warning: This will replace all current configurations. Make sure you have a backup.
                  </p>
                )}
              </div>
            </AlertDialogDescription>
      ...
      <AlertDialog open={showExportDialog} onOpenChange={setShowExportDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground">Choose how to retrieve your exported backup</p>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    onClick={() => {
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      if (!value) return;
                      // Primary attempt: async clipboard API
                      try {
                      ...
                      // Secondary attempt: ClipboardItem (larger payloads)
                      try {
                        // @ts-ignore ClipboardItem may not exist
                ...
                <div>
                  <p className="text-sm mb-2">Base64 Preview</p>
                  <textarea
                    readOnly
                    value={exportBase64}
                    rows={6}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <p className="space-y-2">
                This will replace all configuration with a known blank config.
              </p>
              <p className="text-sm text-amber-600 dark:text-amber-500">
